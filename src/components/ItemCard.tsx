...
import { formatTime, formatCurrency, getCategoryEmoji, cn } from '../lib/utils';
...
export function ItemCard({ item, tripCurrency, onUpdate, onDelete }: ItemCardProps) {
  ...
  const [localLocation, setLocalLocation] = useState(item.location_name || '');
  const [localNotes, setLocalNotes] = useState(item.notes || '');
  const [localBookingRef, setLocalBookingRef] = useState(item.booking_ref || '');
  const [localUrl, setLocalUrl] = useState(item.url || '');
  const [localCost, setLocalCost] = useState(item.cost_estimate?.toString() || '');

  const debouncedUpdate = useDebouncedCallback(
    (updates: Partial<ItineraryItem>) => onUpdate(item.id, updates),
    500
  );
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3 py-3">
          ...
          <button
            ...
            title={`${statusLabels[item.status]} — click to change`}
          >
            <StatusIcon size={20} />
          </button>
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Time & location summary — uses consistent 12h format */}
            ...
            <div className="grid grid-cols-2 gap-3 mt-3 text-xs">
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    debouncedUpdate({ start_time: e.target.value || null });
                  }}
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    debouncedUpdate({ end_time: e.target.value || null });
                  }}
                  className="mt-0.5 w-full px-2 py-1.5 rounded-lg border border-cream-dark bg-cream/30
                    text-sumi text-xs focus:outline-none focus:ring-1 focus:ring-indigo/20"
                />
              ...
              <label className="block col-span-2">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    debouncedUpdate({ location_name: e.target.value || null });
                  }}
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(e) => {
                    setLocalBookingRef(e.target.value);
                    debouncedUpdate({ booking_ref: e.target.value || null });
                  }}
                  className="mt-0.5 w-full px-2 py-1.5 rounded-lg border border-cream-dark bg-cream/30
                    text-sumi text-xs focus:outline-none focus:ring-1 focus:ring-indigo/20"
                />
      ...
      {showDeleteConfirm && (
        <ConfirmDialog
          title="Delete item"
          message={`Remove "${item.title}" from this day? This can't be undone.`}
          confirmLabel="Delete"
          destructive
          onConfirm={() => {
